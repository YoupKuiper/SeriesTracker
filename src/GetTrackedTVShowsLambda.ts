...
export const handler = async (event: any, context: any) => {
	...
	try {
		...
		const showsToReturn = user.trackedTVShows.filter((tvShow: TVShow) =>
			tvShow.name.toLowerCase().includes(searchString.toLowerCase())
		);
		...
		if (error instanceof jwt.JsonWebTokenError) {
			return sendTokenErrorResponse({ message: "Invalid token" });
		}
		return sendErrorResponse({ message: "Failed to get tracked TV Shows" });
	}
};
