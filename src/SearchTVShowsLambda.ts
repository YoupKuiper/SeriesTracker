...
export const handler = async (event: any, context: any) => {
	...
	try {
		...
		if (parsedEvent.searchString) {
			...
			const allEnglishSpokenTVShows = allFetchedPopularTVShows.filter(
		...
		return sendErrorResponse({ message: "Failed to get searched TV Shows" });
	}
};

const getPopularTVShowsForPageNumber = async (number: number) => {
	const response = await axios.get(
		`https://api.themoviedb.org/3/tv/popular?api_key=${process.env.THE_MOVIE_DB_TOKEN}&language=en-US&page=${number}`
	);
...
const searchTVShows = async (title: number) => {
	const response = await axios.get(
		`https://api.themoviedb.org/3/search/tv?api_key=${process.env.THE_MOVIE_DB_TOKEN}&query=${encodeURIComponent(
			title
		)}`
	);
	return response.data.results;
...
const getDetailsForTVShow = async (id: number) => {
	const response = await axios.get(
		`https://api.themoviedb.org/3/tv/${id}?api_key=${process.env.THE_MOVIE_DB_TOKEN}&append_to_response=recommendations`
	);
